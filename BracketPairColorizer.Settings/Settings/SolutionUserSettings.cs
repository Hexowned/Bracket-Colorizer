...
{
    internal class SolutionUserSettings : ISolutionUserSettings
    {
        private readonly IPersistSettings persistSettings;
        ...
        private T GetEntry<T>(JObject json, string key) where T : ISettingsObject, new()
        {
            ...
            if (keyEntry == null) { return default(T); }

            T settingsObject = new T();
            var settingsEntry = keyEntry[settingsObject.Name];
            if (settingsEntry == null) { return default(T); }

            DeserializeEntry((JObject)settingsEntry, settingsObject);
        ...
        private JObject TryRead()
        {
            ...
            {
                // this will avoid generating a VS error if the JSON stored is invalid.
            }
        ...
        private void DeserializeEntry<T>(JObject settingsEntry, T settingsObject) where T : ISettingsObject
        {
            string text = settingsEntry.ToString();
            using (var jr = new JsonTextReader(new StringReader(text)))
            {
                settingsObject.Read(jr);
            }
        }
    }
}
