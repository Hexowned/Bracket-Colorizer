...
namespace BracketPairColorizer.Core.Outlining
{
    public abstract class BaseOutliningManager : IUserOutlining, IOutliningManager
    {
        ...
        {
            this.outliningTagger.RaiseRagsChanged(span);
            this.glyphTagger.RaiseRagsChanged(span);
        }
        ...
        private class OutliningTagger : ITagger<IOutliningRegionTag>
        {
            ...
            public IEnumerable<ITagSpan<IOutliningRegionTag>> GetTags(NormalizedSnapshotSpanCollection spans)
            {
                return from span in this.manager.GetTags(spans)
                       select new TagSpan<IOutliningRegionTag>(span, new OutliningRegionTag(false, false, "...", span.GetText()));
            }

            public void RaiseRagsChanged(SnapshotSpan span)
            {
