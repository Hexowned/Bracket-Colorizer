...
namespace BracketColorizer.Bracket
{
    public class Classifier : IClassifier
    {
        ...
        {
            this._classificationTypeRegistryService = registry;
        }
        ...
        public IList<ClassificationSpan> GetClassificationSpans(SnapshotSpan span)
        {
            ...
            if (bracket.Where(s => s.Item3 == '(').ToList().Count != bracket.Where(s => s.Item3 == ')').ToList().Count)
            {
                return spans;
            }
            ...
            while (bracket.Count > 0)
            {
                ...
                IClassificationType classificationType = _classificationTypeRegistryService.GetClassificationType(_tp);
                ...
                SnapshotSpan bracketStartSpan = new SnapshotSpan(span.Snapshot, new Span(lineStart.Start + line1.Item2, 1));
                SnapshotSpan bracketEndSpan = new SnapshotSpan(span.Snapshot, new Span(lineEnd.Start + line2.Item2, 1));

                if (classificationType != null)
                {
                    spans.Add(new ClassificationSpan(bracketStartSpan, classificationType));
                    spans.Add(new ClassificationSpan(bracketEndSpan, classificationType));
                }
            }
