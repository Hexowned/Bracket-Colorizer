...
{
    internal class RainbowColorTagger : ITagger<RainbowTag>
    {
        ...
        {
            if (this.provider?.Settings == null) { yield break; }

            if (!IsEnabled() || spans.Count == 0) { yield break; }

            var braceCache = this.provider.BufferBraces;
            ITextSnapshot snapshot = spans[0].Snapshot;
            if (braceCache == null || !braceCache.Enabled) { yield break; }
            if (braceCache.Snapshot != spans[0].Snapshot) { yield break; }
            ...
            {
                yield return new TagSpan<RainbowTag>(new SnapshotSpan(snapshot, error.Position, 1), new RainbowTag(this.rainbowError));
            }
        }
        ...
        {
            return this.provider.Settings.RainbowTagsEnabled && this.provider.Settings.RainbowColorize;
        }
    }
}
