...
namespace BracketPairColorizer.Rainbow
{
    ...
    public class RainbowHighlightProvider : IWpfTextViewCreationListener
    {
        [Export(typeof(AdornmentLayerDefinition))]
        [Name(RainbowHighlight.LAYER)]
        [Order(After = PredefinedAdornmentLayers.Text, Before = PredefinedAdornmentLayers.InterLine)]
    ...
    {
        public const string LAYER = "BracketPairColorizer.rainbow.highlight";
        ...
        private readonly RainbowHighlightProvider provider;

        public RainbowHighlight(IWpfTextView textView, RainbowHighlightProvider provider)
        {
            this.view = textView;
        ...
        public void Start(SnapshotPoint opening, SnapshotPoint closing, int depth)
        {
            ...
            this.layer.AddAdornment(AdornmentPositioningBehavior.TextRelative, span, TAG, adornment, null);
        }
        ...
        private Brush GetRainbowBrush(int depth)
        {
            ...
            var props = this.formatMap.GetTextProperties(rainbow);

            return props.ForegroundBrush;
        }

        private Brush MakeBackgroundBrush(Brush brush)
        {
            if (!(brush is SolidColorBrush scb)) return Brushes.Transparent;

            return new SolidColorBrush(scb.Color) { Opacity = 0.10 };
        }
    }
}
